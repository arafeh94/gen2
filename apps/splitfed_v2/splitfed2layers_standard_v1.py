...
random.seed(42)
configs = json.loads(sys.argv[1]) if len(sys.argv) > 1 else {'name': '2layers_standard_v1'}
logger = SQLiteLogger.new_instance('splitlearn_v2_1.sqlite', configs)
printer = logging.getLogger('2layers_standard_v1')
# configs
...
for cluster_speed in cluster_speeds:
    servers[cluster_speed] = Server(copy.deepcopy(server_model), copy.deepcopy(client_model), test_data, lr=lr_server)
internal_counter = 0
for round_id in range(rounds):
    ...
    while True:
        ...
        stats = splitlearn.one_round_resource(run_clusters, servers[speed_idx], is_parallel=True, is_selection=False,
