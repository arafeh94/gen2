...
from apps.experiments.fedprox.modules import FedProxTrainer
...
random.seed(42)
configs = Dict(json.loads(sys.argv[1])) if len(sys.argv) > 1 else Dict({'name': 'fed_prox'})
print(configs)
logger = SQLiteLogger.new_instance('splitlearn_v3.sqlite', configs)
printer = logging.getLogger('fed_prox')
# configs
...
trainer_params = TrainerParams(trainer_class=FedProxTrainer, batch_size=batch, epochs=epochs, optimizer='sgd',
