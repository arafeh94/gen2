...
from apps.donotuse.main_split.client import Client
...
def from_clients(clients_data, initial_model, size_cluster=1) -> dict[Cluster]:
    clusters = {}
    for i in range(0, len(clients_data), size_cluster):
        ...
        clusters[i] = Cluster(copy.deepcopy(initial_model), cluster_clients)
    return clusters
