...
# import configs
import configs_new as configs
from configs_new import distributor
...
for cfid, config in configs.runs.items():
    db_logger = SQLiteLogger.new_instance('seqfed3.sqlite', config)
    ...
    if method.startswith('seqop'):
        ...
        for r, ((acc, loss, f1), time) in enumerate(zip(acc_loss, times)):
            db_logger.log(r, acc=acc, loss=loss, time=time, selector=selection_history, f1=f1)
        ...
        for r, acc in enumerate(test_acc):
            db_logger.log(r, test_acc=acc / 100, acc=acc_loss[0], loss=acc_loss[1], f1=acc_loss[2],
                          time_taken=timer.cp(-1))
    ...
    pickle.dump(initial_weights, open(f'./weights/{db_logger.id}.pkl', 'wb'))
    db_logger.close()
