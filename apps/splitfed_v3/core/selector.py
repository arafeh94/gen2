...
from apps.splitfed_v3.core.reinforcement_learning import ReinforcementLearningEnv
...
def re_cluster(cls):
    ...
    for cluster in cls:
        for client in cluster:
            if client.configs['group_id'] not in client_clusters:
                client_clusters[client.configs['group_id']] = Cluster(client.model_copy(), [client])
            else:
                client_clusters[client.configs['group_id']].append(client)
...
# if I have 10 non-iid cluster, this method will be called 10 times
def ri_selection(one_cluster: Cluster, size=0.1):
    deny_all(one_cluster)
    ri_env = ReinforcementLearningEnv()
    # should I do reset on every selection?
    # should I use client ID as index? (client id is a random unique identifier generated for each client)
    ri_env.reset(one_cluster.clients)
    for client in ri_env:
        # what action should I pass first?
        action = ri_env.step(None, None)
        # I can't get any realtime info right now
        # real time info of client? or all of the clients?
        real_time_info = {}
        client.configs['group_id'] = ri_env.step(action, client)
